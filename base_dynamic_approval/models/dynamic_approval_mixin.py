...
class DynamicApprovalMixin(models.AbstractModel):
            ...
            for record in self:
                try:
                    record.with_context(mail_activity_quick_update=True).activity_schedule(
                        ...
                        summary='تمت الموافقة على الطلب في نظام موارد.',
                        user_id=user.id,
                    )
                except Exception as error_message:
                    _logger.exception(
        ...
        if activity_type:
            for record in self:
                try:
                    record.with_context(mail_activity_quick_update=True).activity_schedule(
                        activity_type_id=activity_type.id,
                        summary='تم استرجاع الطلب في نظام موارد.',
                        user_id=user.id,
                    )
                except Exception as error_message:
                    _logger.exception(
        ...
        if activity_type:
            for record in self:
                try:
                    record.with_context(mail_activity_quick_update=True).activity_schedule(
                        activity_type_id=activity_type.id,
                        summary='تم اعتماد الطلب في نظام موارد.',
                        user_id=user.id,
                    )
                except Exception as error_message:
                    _logger.exception(
    ...
    def _get_pending_approvals(self, user):
        ...
        for request in self.dynamic_approve_request_ids.filtered(
                lambda request_approve: request_approve.status in ['pending']):

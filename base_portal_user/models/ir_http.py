...
class Http(models.AbstractModel):
    ...
    def session_info(self):
        ...
        menu_json_utf8 = json.dumps(
            {str(k): v for k, v in request.env['ir.ui.menu'].load_menus(request.session.debug).items()}, default=ustr,
        ...
        session_info.update({
            "user_companies": {'current_company': user.company_id.id,
                               'allowed_companies': {comp.id: {'id': comp.id, 'name': comp.name} for comp in
                                                     user.company_ids}},

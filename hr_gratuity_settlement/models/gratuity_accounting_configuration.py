...
class GratuityAccountingConfiguration(models.Model):
    ...
    furniture_configuration_table = fields.One2many('furniture.configuration',
                                                    'furniture_accounting_configuration_id')
    employment_category = fields.Selection(
        [('perm_in_house', 'Permanent In house'),
         ('perm_staff', 'Permanent Staff')], default="perm_staff", required=True,
        string='Employment Category')
    nationality = fields.Selection(
        [('QA', 'Qatari'),
         ('expatriate', 'Expatriate')], default="QA", required=True,
        string='Nationality')

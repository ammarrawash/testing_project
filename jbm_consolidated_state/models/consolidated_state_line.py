...
class ConsolidatedStateLine(models.Model):

    _name = 'consolidated.state.line'
    ...
    consolidated_id = fields.Many2one('consolidated.state')
    field_id = fields.Many2one('ir.model.fields', string="State Field")
    field_state_id = fields.Many2one('ir.model.fields.selection', string="State Field Value")
    state = fields.Selection([
        ...
        ('done', 'Done'),
        ('cancelled', 'Cancelled'),
    ], string="Consolidated State")
    default_state = fields.Boolean(string='Default State')
    time_in_days = fields.Float(string="Time In Days")
    ...
    model = fields.Char(related='consolidated_id.model_id.model', store=True)

    # model_condition_id = fields.Many2one(
    #     comodel_name='consolidated.model.condition',
    # )
    ...
    def _is_condition_matched_python_code(self, res):
        """ return true / false based on a calculation type """
        self.ensure_one()
        try:
            localdict = {
                # 'datetime': safe_eval.datetime,
